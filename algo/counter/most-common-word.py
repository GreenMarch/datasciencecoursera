...
#         return my_out[0][0]
        ...
        print(c)
        #while c:
            #top_word = max(c, key = lambda x:c.get(x))
        print("banned_set: " + str(sorted(set(banned))))
        for word in c:
            if word not in set(banned) and c.get(word) > most_common:
                out = word
                most_common = c.get(word)
        return out
